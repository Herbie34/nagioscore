...
static int cmd_submitf(int id, const char *fmt, ...) {
	...
	len = snprintf(cmd, sizeof(cmd), "[%llu] %s;", (unsigned long long)time(NULL), command_name);
...
int commit_command(int cmd) {
	...
	switch(cmd) {
		...
		case CMD_DELAY_HOST_NOTIFICATION:
			result = cmd_submitf(cmd, "%s;%llu", host_name, (unsigned long long)notification_time);
			break;

		case CMD_DELAY_SVC_NOTIFICATION:
			result = cmd_submitf(cmd, "%s;%s;%llu", host_name, service_desc, (unsigned long long)notification_time);
			break;
		...
		case CMD_SCHEDULE_FORCED_SVC_CHECK:
			...
			result = cmd_submitf(cmd, "%s;%s;%llu", host_name, service_desc, (unsigned long long)start_time);
			break;
		...
		case CMD_SHUTDOWN_PROCESS:
		case CMD_RESTART_PROCESS:
			result = cmd_submitf(cmd, "%llu", (unsigned long long)scheduled_time);
			break;
		...
		case CMD_SCHEDULE_HOST_SVC_CHECKS:
			...
			result = cmd_submitf(cmd, "%s;%llu", host_name, (unsigned long long)scheduled_time);
			break;
		...
		case CMD_SCHEDULE_HOST_DOWNTIME:
			...
			result = cmd_submitf(cmd, "%s;%llu;%llu;%d;%lu;%lu", host_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, triggered_by, duration);
			break;
		...
		case CMD_SCHEDULE_SVC_DOWNTIME:
			result = cmd_submitf(cmd, "%s;%s;%llu;%llu;%d;%lu;%lu", host_name, service_desc, (unsigned long long)start_time, (unsigned long long)end_time, fixed, triggered_by, duration);
			break;
		...
		case CMD_SCHEDULE_HOST_CHECK:
			...
			result = cmd_submitf(cmd, "%s;%llu", host_name, (unsigned long long)start_time);
			break;
		...
		case CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME:
			result = cmd_submitf(cmd, "%s;%llu;%llu;%d;0;%lu", hostgroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			break;

		case CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME:
			result = cmd_submitf(cmd, "%s;%llu;%llu;%d;0;%lu", hostgroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			if(affect_host_and_services == TRUE)
				result |= cmd_submitf(CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME, "%s;%llu;%llu;%d;0;%lu", hostgroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			break;
		...
		case CMD_SCHEDULE_SERVICEGROUP_HOST_DOWNTIME:
			result = cmd_submitf(cmd, "%s;%llu;%llu;%d;0;%lu", servicegroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			break;

		case CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME:
			result = cmd_submitf(cmd, "%s;%llu;%llu;%d;0;%lu", servicegroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			if(affect_host_and_services == TRUE)
				result |= cmd_submitf(CMD_SCHEDULE_SERVICEGROUP_HOST_DOWNTIME, "%s;%llu;%llu;%d;0;%lu", servicegroup_name, (unsigned long long)start_time, (unsigned long long)end_time, fixed, duration);
			break;
